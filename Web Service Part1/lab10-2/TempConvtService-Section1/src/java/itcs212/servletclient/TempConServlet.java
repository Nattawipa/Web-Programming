...
import itcs212.HelloWorldWS_Service;
...
public class TempConServlet extends HttpServlet {
    ...
    private String celsiusToFahrenheit(double celsius) {
        ...
        itcs212.HelloWorldWS port = service.getHelloWorldWSPort();
        return port.celsiusToFahrenheit(celsius);
    }

    private String getMydetail(java.lang.String id) {
        // Note that the injected javax.xml.ws.Service reference as well as port objects are not thread safe.
        // If the calling of port operations may lead to race condition some synchronization is required.
        itcs212.HelloWorldWS port = service.getHelloWorldWSPort();
        return port.getMydetail(id);
    }
